flytable_meta <- function(){
  ft=fafbseg::flytable_query("select _id, root_id, supervoxel_id, proofread, status, pos_x, pos_y, pos_z, nucleus_id, side, top_nt, top_p, ito_lee_hemilineage, hartenstein_hemilineage, cell_type, hemibrain_match, root_duplicated from info")
  ft = as.data.frame(ft)
  rownames(ft) = ft$root_id
  ft
}

flytable_matches <- function(dataset = NULL){
  ft=fafbseg::flytable_query("select _id, id, side, cell, cell_type,cell_body_fiber, ito_lee_hemilineage, match, quality, fafb_hemisphere_match, fafb_hemisphere_match_quality, dataset, match_dataset, user from flywire_matches")
  if(!is.null(dataset)){
...
flytable_matches_update <- function(df, DryRun = FALSE){
  ft=fafbseg::flytable_query("select _id, id, side, cell, cell_type,cell_body_fiber, ito_lee_hemilineage, match, quality, fafb_hemisphere_match, fafb_hemisphere_match_quality, dataset, match_dataset, user from flywire_matches")
  toupdate = setdiff(df, ft)
  if(DryRun)
    toupdate
  else {
    fafbseg::flytable_update_rows(toupdate, table = "flywire_matches")
    invisible(toupdate)
  }
}
