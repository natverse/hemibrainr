---
title: "axons_dendrites"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{axons_dendrites}
...
Other compartments include the 'primary neurite' or 'cell body fibre' (purple, beow) [(Ito et al., 2014)](https://www.sciencedirect.com/science/article/pii/S0896627313011781), the non-synaptic cable that attaches the neuron's cell body, its 'soma' to the rest of the neuron. There is also a 'primary dendrite', or 'linker' (green, below), which is non-synaptic cable that connects a neuron's dendrite and axon, throguh which an action potential presumably travels.
...
Splitting neurons can take a little time. We can also load thousands of pre-split neurons from the `hemibrainr` Google team drive. In order to connect R to this Google drive, you have a few options. Please see [this article](https://flyconnectome.github.io/hemibrainr/articles/google_filestream.html). Once you have access and have moutned the drive, you should be able to do:
...
Here, there is one entry per synapse. The prepost column tells us whether the synapse is presynaptic (0, an outout synapse) or postsynaptic (1, an input synapse).
...
Here, pre is the `bodyid` of the presynaptic (upstream source) neuron and post is the `bodyid` of the postsynaptic (downstream target) neuron. The `norm` column gives a synaptic weight normalised by the totla inputs of the downstream neuron (post), i.e. count/total postsynapses on 'post'.
